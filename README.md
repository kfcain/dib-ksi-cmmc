...
- **11 invariants** ([`ontology/dib-ksi-ontology.json`](ontology/dib-ksi-ontology.json)) — what the software refuses to let you claim: it rejects self-graded status, aggregate coverage percentages, unverified inheritance, hand-typed deliverables, unsigned attestations treated as signed, and evidence depth reported met without class evidence-type floors. [docs/05-invariants](docs/05-invariants.md).
